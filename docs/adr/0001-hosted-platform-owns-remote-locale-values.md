...
Local mode and remote mode have different Locale value owners. In local mode, the Consumer app repo owns generated Runtime bundles and local `translate` fills those bundles; in remote mode, the hosted platform owns Locale values and package-local translation callbacks are not canonical.
