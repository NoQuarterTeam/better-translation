...
Feature Branch values must not overwrite `main` automatically. A Branch override stores a branch-specific Locale value together with the parent value hash it was based on, so future reconciliation can detect whether `main`, the feature branch, or both changed after branching.
