...
import { updateProjectIconVersion, withProjectIconUrl } from "@/server/profile-images"
import { deleteStorageObject, getProjectIconKey, headStorageObject } from "@/server/storage"
...
export const confirmProjectIconUploadFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    await updateProjectIconVersion(context.project.id)

    const project = await db.query.projectsTable.findFirst({ where: { id: context.project.id } })
    if (!project) throw new Error("Project not found.")

    return getProjectSettings(project, context.organization)
  })

export const removeProjectIconFn = createServerFn({ method: "POST" })
  .middleware([projectMiddleware])
  .handler(async ({ context }) => {
    await deleteStorageObject(getProjectIconKey(context.project.id)).catch(() => undefined)
    ...
    if (!project) throw new Error("Could not remove Project icon.")
...
async function getProjectSettings(project: typeof projectsTable.$inferSelect, organization: { id: string; slug: string }) {
  ...
  const projectWithIconUrl = await withProjectIconUrl(project)

  return {
    icon: projectWithIconUrl.icon,
    iconUrl: projectWithIconUrl.iconUrl,
    id: project.id,
    name: project.name,
