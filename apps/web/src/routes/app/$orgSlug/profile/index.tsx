...
export const Route = createFileRoute("/app/$orgSlug/profile/")({
...
function ProfilePage() {
  ...
  const linkProvider = useMutation({
    mutationFn: async (provider: AuthProvider) => {
      const result = await authClient.linkSocial({ provider, callbackURL: `/app/${orgSlug}/profile` })
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 overflow-auto p-4 md:p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          <T>Manage your account details and sign-in methods.</T>
        </p>
      </div>

      <div className="grid max-w-5xl gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,0.8fr)]">
        <Card>
          <CardHeader>
            <CardTitle>
              <T>Your details</T>
